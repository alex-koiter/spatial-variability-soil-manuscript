...
```{r}
features <- usdm::vifstep(rast(select(filter(train, dataset == "train"), -dataset, -co)), th = 8)
features
```
...
orig_data <- read_csv(here::here("./notebooks/forest_data_49.csv"))  %>%
  select("co", any_of(attribute))

prediction_org <- predict(rf.fit, newdata = orig_data)
test_plot_org <- data.frame(pred = prediction_org, obs = orig_data$co)
