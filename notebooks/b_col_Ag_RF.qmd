...
```{r}
features <- usdm::vifstep(rast(select(filter(train, dataset == "train"), -dataset, -b_c)), th = 8)
features
```
...
#caret::postResample(test_plot$pred, test_plot$obs)['RMSE']^2
...
orig_data <- read_csv(here::here("./notebooks/ag_data_49.csv"))  %>%
  select("b_c", any_of(attribute))

prediction_org <- predict(rf.fit, newdata = orig_data)
test_plot_org <- data.frame(pred = prediction_org, obs = orig_data$b_c)
