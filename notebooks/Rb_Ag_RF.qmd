...
```{r}
features <- usdm::vifstep(rast(select(filter(train, dataset == "train"), -dataset, -rb)), th = 8)
features
```
...
orig_data <- read_csv(here::here("./notebooks/ag_data_49.csv"))  %>%
  select("rb", any_of(attribute))

prediction_org <- predict(rf.fit, newdata = orig_data)
test_plot_org <- data.frame(pred = prediction_org, obs = orig_data$rb)
